...
class Combined(nn.Module):            
    ...
    def forward(self, x, y):
        ...
        model2_output = self.model2(scaled_y)
...
def icgl_train_generator_AC2D(nc,dom):
    ...
    uxi = torch.sin(2*torch.pi*xi)
    uyi = torch.cos(2*torch.pi*yi)
...
def closure():
    ...
    # Compute loss
    loss = spinn_loss(spinn, spinn, train_data_gauss)
...
hidden_sizes = [64, 64, 64, 64, 64]  # You can specify the number of neurons in each hidden layer
output_size = 1022
epochs_icgl = 1001
epochs_pinn = 2001
tau = 1e-6
...
for i in range(10):
    xi, yi, ui = train_data_icgl
    ui = spinn(xi, yi)
    ui.detach_()
    train_data_icgl = xi, yi, ui
...
torch.save(checkpt,'AC_GELU_Working.pt')
...
plt.figure()
plt.pcolor(xmesh, ymesh, ypred.detach().numpy(), label ='Predicted')
plt.colorbar()
...
_, _, xg, yg, _, _, h = spinn_train_generator_AC2D(512,[-1,1])
